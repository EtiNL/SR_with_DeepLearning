...
class run:
    ...
    def test(self, path):
        ...
        cropped = fullimg[0:(width - (width % self.scale)), 0:(height - (height % self.scale)), :] #cropping the image for it to be in shape [scale*n, scale*m]
        img = cv2.resize(cropped, None, fx=1. / self.scale, fy=1. / self.scale, interpolation=cv2.INTER_CUBIC) #resizing the image with cubic interpolation for it to be in shape [n, m]

        # to ycrcb and normalize
        # explanations on ycrcb on https://www.kenrockwell.com/tech/sycc.htm#:~:text=YCC%20transmits%20or%20represents%20the,simply%20YCC%20created%20from%20sRGB.
        # y is called Luminance and cr cb are called Color difference signals
        img_ycc = cv2.cvtColor(img, cv2.COLOR_BGR2YCrCb)
        img_y = img_ycc[:,:,0] #img_y is the Luminance
        floatimg = img_y.astype(np.float32) / 255.0 #normalization of the Luminance
